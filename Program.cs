...
namespace PGPENCYPTION
{
    class Program
    {
        ...
        {
            String Password = "redcrossKCB";
            String Identity = "redcrossKCB";

            PgpKeyRingGenerator krgen = generateKeyRingGenerator(Identity, Password);

            // Generate public key ring, dump to file.
            PgpPublicKeyRing pkr = krgen.GeneratePublicKeyRing();
            BufferedStream pubout = new BufferedStream(new FileStream(@"D:\New folder\Publickey.pkr", System.IO.FileMode.Create));
            pkr.Encode(pubout);
            pubout.Close();

            // Generate private key, dump to file.
            PgpSecretKeyRing skr = krgen.GenerateSecretKeyRing();
            BufferedStream secout = new BufferedStream(new FileStream(@"D:\New folder\PrivateKey.skr", System.IO.FileMode.Create));
            skr.Encode(secout);
            secout.Close();
